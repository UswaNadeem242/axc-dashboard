...
export function SalesBillingPanel({
  ...
  const handleRemoveCharge = (key: ChargeKey) => {
          ...
          <div className="flex flex-col gap-1">
            ...
            <CommonDropdown
              ...
              onChange={(val: string[]) => {
                ...
                const addedCharges = val.filter(x => CHARGE_ROWS.some(r => r.key === x) && !visibleCharges.has(x as ChargeKey));
                const addedDynamicCharges = val.filter(x => billing.dynamicCharges?.[x] && !visibleDynamicCharges.has(x));

                removedCharges.forEach(key => handleRemoveCharge(key as ChargeKey));
                removedDynamicCharges.forEach(key => handleRemoveDynamicCharge(key));

                addedCharges.forEach(key => {
                  const k = key as ChargeKey;
                  setVisibleCharges(prev => {
                    const next = new Set(prev);
                    next.add(k);
                    return next;
